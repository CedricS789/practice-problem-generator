...
Grounded Problems turns an active note, editor selection, or explicit PDF page range into source-grounded practice problems. Draft generation runs through an AI command-line tool that you install and select on desktop; saved banks, dashboards, and study sessions work on desktop and mobile.
...
Once the plugin is listed in Obsidian's Community Plugins directory, search for **Grounded Problems**, install it, and enable it.

For manual installation, download `main.js`, `manifest.json`, and `styles.css` from the matching [GitHub release](https://github.com/CedricS789/grounded-problems/releases), place them in `<vault>/.obsidian/plugins/practice-lab-ai/`, and enable **Grounded Problems** under **Settings → Community plugins**. Keep the folder name `practice-lab-ai` so existing settings and banks remain compatible.
...
- Provides in-place sliders and precise whole-number inputs for every exercise type. Changing one share automatically rebalances the other selected types to 100%, displays deterministic item counts, and enforces those exact counts in provider output without resetting the Grounded Problems pane.
...
- Provides a full settings control center for provider-specific model defaults, reasoning, reusable focus instructions, automatic local-image/GIF selection, study order, free-response review, compact or comfortable density, and granular visibility of Grounded Problems, saved-bank, and dashboard statistics.
...
- `Grounded Problems: Generate from selection`
- `Grounded Problems: Generate from current note`
- `Grounded Problems: Generate from current PDF`
- `Grounded Problems: Open workspace`
- `Grounded Problems: Start practice for current note`
- `Grounded Problems: Start practice for current PDF`
- `Grounded Problems: Open practice dashboard`
...
Settings are split into generation defaults, PDF source defaults, study defaults, Grounded Problems view, saved-bank statistics, dashboard, and advanced runtime sections. New sources can inherit a provider, provider-specific model, reasoning level, item count, difficulty, exact exercise percentages, focus instructions, GIF frame, and manual or automatic local-image/GIF selection. PDF defaults control the initial page window, maximum pages, maximum extracted characters, extraction timeout, and exact `pdfinfo` / `pdftotext` executables. Codex and Claude model defaults may be left blank to use their provider default; agy preserves its installed Grounded Problems default until changed. New sessions can retain bank order or shuffle a copy without changing the saved bank. Keyboard shortcuts, automatic answer-field focus, and the visible shortcut hint are independently configurable.
...
Folder paths are the stable organization model. Tags are read dynamically from the current source note, including frontmatter and inline tags; a parent tag also includes its nested tags. Grounded Problems does not copy tags into the bank or require a special dashboard tag, so changing a source note's taxonomy changes the next dashboard refresh without rewriting practice history.
...
Completed-session statistics are calculated locally from the history already stored in the practice Markdown. Correct answers earn one point, partially correct free responses earn half a point, and incorrect answers earn zero. Objective, self-assessed, and AI-reviewed provenance remain distinct. Pending and failed reviews count toward completion but not points, rank, or streak. Grounded Problems does not send telemetry.

- Codex runs ephemerally in a read-only sandbox with a JSON output schema and neutral image copies.
- Claude runs in non-persistent print mode with a JSON schema and access restricted to the isolated job directory.
- agy supports text generation. Vision remains unavailable until **Settings → Grounded Problems → Test agy vision** completes its synthetic one-pixel headless probe successfully; no vault content is used by that test.

One CLI child process may run at a time, including provider detection and capability probes. Generation supports targeted cancellation and has a five-minute default timeout. Background answer reviews wait without interrupting practice, use their separately configurable timeout, and retry one retryable process failure. A transient bank-write failure uses bounded backoff while retaining the finished result for later retry. Every structured job allows one schema-repair attempt and uses neutral files under the operating-system temporary directory. Temporary jobs are removed after completion.

## Privacy and data handling

Grounded Problems has no telemetry and no background conversion. It never sends a note merely because the note was opened or saved.

Generation and optional AI answer review are explicit networked actions. After you inspect and approve the payload, the plugin starts the selected locally installed Codex, Claude, or agy executable. That CLI may send the approved source text, neutral media copies, answers, and bounded grading context to its provider. Grounded Problems does not silently switch providers. Provider accounts, terms, retention policies, and possible usage charges are managed by the selected CLI provider; the plugin itself is free and open source.
...
Banks created under the former display name remain readable, including snapshots under `_Vault/Attachments/Practice Lab/`. New snapshots use the Grounded Problems folder; the plugin does not move or rewrite legacy material automatically.
...
A PDF under the same course hierarchy uses that course's `Practice/` folder. A PDF elsewhere in the vault uses a deterministic collision-safe bank under `Notes/Practice Sources/Practice/`; Grounded Problems never writes beside a mirrored or read-only source PDF.
...
Grounded Problems does not perform on-save or whole-vault conversion, crawl or automatically ingest linked PDFs, run OCR, create a spaced-repetition or flashcard queue, schedule reviews with FSRS, export to Anki, run a localhost service, or collect telemetry.
