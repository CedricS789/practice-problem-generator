...
- Shows a collapsible live agent-activity panel for generation and background answer review, with elapsed time, schema-repair state, provider progress, safe emitted reasoning status, and token/cost metadata. Provider-reported usage is distinguished from local text estimates; local estimates exclude provider-specific visual tokenization. The live event list is ephemeral and never displays raw provider output or private chain-of-thought, while the final safe generation totals are retained with generation provenance.
...
- Uses progressive disclosure throughout creation and study: the current task and one primary action remain visible, while generation-engine settings, manual mix controls, source galleries, exact payloads, activity, analytics, provenance, and destructive controls stay available behind consistently named disclosures.
- Lets you confirm each approved source as a personal note, official correction, instructor material, assigned reference, or unclassified source. Configurable folder and tag rules can suggest labels, but suggestions never become course authority until you confirm them.
- Optionally compares personal-note claims with explicitly approved school material before Guided generation. Official corrections outrank instructor material, assigned references, and personal notes; disagreements remain visible and cannot be settled silently from model knowledge. Immutable comparison snapshots appear only after an answer and never change the score.
...
- Start with one primary note, selection, or exact PDF page range and optionally add up to four supporting notes or PDF ranges. Every source is chosen explicitly: the plugin never crawls links, expands a PDF range, or fills a source gap from general knowledge. All selected PDFs share the configured total page and extracted-character budgets; the source screen shows used and remaining capacity, each additional picker receives only the remainder, and the controller checks the totals again before every new provider job.
...
Interrupted-generation recovery does not use a provider conversation or start a replacement model turn. Codex remains ephemeral and Claude remains non-persistent. The plugin stores only a small recovery handle in its settings; the exact approved prompt, schema, neutral media copies, provider output, and validated draft checkpoint stay in the isolated operating-system temporary job. On desktop startup, provider discovery begins immediately rather than waiting for a potentially long recovery; the shared coordinator still prevents detection, recovery, and queued AI reviews from running CLI children at the same time. Until the draft is saved or explicitly discarded, new generation is blocked rather than silently overwriting it.
...
Generation, guided-path creation, regeneration, PDF extraction, media extraction, and executing AI answer reviews are desktop-only because they start locally installed command-line programs. Their commands, context-menu actions, guided creation view, provider controls, and generation-only settings are not shown on mobile; the dual-purpose practice view displays only its study surface there. The plugin itself is not desktop-only: the dashboard and saved `practice-lab` blocks render on mobile, all exercise types can be completed offline, LaTeX renders through Obsidian MathJax, occlusion feedback reveals the unmasked local image, and session history is written in one revision-aware batch at completion.
...
The live activity panel is derived locally from provider event metadata. It shows elapsed time, bounded status labels, response-size progress, input/output usage, cache and reasoning tokens when exposed, schema-repair attempts, and batch totals. A `~` marks locally estimated text tokens; visual tokenization is excluded from that estimate. Monetary cost is shown only when the selected CLI explicitly reports it, because API list prices may not represent subscription-based CLI billing. Raw prompts, paths, generated answer JSON, tool input, and private reasoning text are never shown. Activity entries are capped in memory and disappear when the view is closed; final safe generation telemetry is stored in the revision-linked generation audit.
...
The note keeps only its useful source wikilink in Obsidian's visible Properties. Schema markers, hashes, bank IDs, revisions, model recipes, generation history, and PDF provenance are plugin-owned and live in the versioned fenced bank or one invisible versioned metadata comment. Legacy frontmatter remains readable and is hidden from the Properties editor immediately; the next authorized bank write consolidates it into the hidden metadata block. The sidecar preserves strict bank-schema compatibility while allowing revision-linked generation history. Bank schema v4 preserves pending, reviewed, and failed answer-review provenance, immutable session evidence, source classifications, alignment records and snapshots, source-material references, and optional learning-path records without introducing scheduling. Legacy v1, v2, and v3 banks validate and migrate losslessly in memory; the first authorized write stores v4 with historical source labels marked unclassified. Unknown schema versions render read-only with recovery instructions.
